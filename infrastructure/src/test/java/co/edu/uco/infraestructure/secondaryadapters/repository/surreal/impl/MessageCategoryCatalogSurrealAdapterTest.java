...
class MessageCategoryCatalogSurrealAdapterTest {
    ...
    private MessageCategoryCatalogSurrealAdapter adapter;

    @BeforeEach
    void setUp() {
        when(loggerFactory.getLogger(MessageCategoryCatalogSurrealAdapter.class)).thenReturn(log);
        adapter = new MessageCategoryCatalogSurrealAdapter(surreal, loggerFactory);
    }
    ...
    private Object categoryDocument(String id, String name) {
        Object doc = mock(Object.class);
        doReturn(recordIdValue("message_category", id)).when(doc).get("id");
    ...
    @Test
    void findAll_mapsCategories() {
        String uuid = UUID.randomUUID().toString();
        doReturn(responseWithOne(categoryDocument(uuid, "GENERAL"))).when(surreal).query(anyString());

        List<MessageCategoryData> result = adapter.findAll();
