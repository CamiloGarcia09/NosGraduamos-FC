...
@Repository
public class MessageCategoryCatalogSurrealAdapter implements MessageCategoryCatalogRepository {
    ...
    public MessageCategoryCatalogSurrealAdapter(final Surreal surreal, final LoggingPortFactory loggerFactory) {
        this.log = loggerFactory.getLogger(MessageCategoryCatalogSurrealAdapter.class);
        this.surreal = surreal;
    }

    @Override
    public List<MessageCategoryData> findAll() {
        final String sql = "SELECT * FROM " + SURREAL_TABLE_MESSAGE_CATEGORY + ";";
        final List<MessageCategoryData> list = new ArrayList<>();
        try {
            ...
            for (int i = 0; i < array.len(); i++) {
                ...
                if (!isNullObject(item) && item.isObject()) {
                    list.add(toMessageCategoryData(item.getObject()));
                }
            }
        } catch (final RuntimeException ex) {
            log.error("Error al consultar categorías de mensaje en SurrealDB: " + sql, ex);
            throw ex;
        }
        return list;
    }

    private static MessageCategoryData toMessageCategoryData(final Object obj) {
