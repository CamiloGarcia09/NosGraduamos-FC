...
@Repository
public class MessageEnvironmentStateCatalogSurrealAdapter implements MessageEnvironmentStateCatalogRepository {
    ...
    public MessageEnvironmentStateCatalogSurrealAdapter(final Surreal surreal, final LoggingPortFactory loggerFactory) {
        this.log = loggerFactory.getLogger(MessageEnvironmentStateCatalogSurrealAdapter.class);
        this.surreal = surreal;
    }

    @Override
    public List<MessageEnvironmentStateData> findAll() {
        final String sql = "SELECT * FROM " + SURREAL_TABLE_MESSAGE_ENVIRONMENT_STATE + ";";
        final List<MessageEnvironmentStateData> list = new ArrayList<>();
        try {
            ...
            for (int i = 0; i < array.len(); i++) {
                ...
                if (!isNullObject(item) && item.isObject()) {
                    list.add(toMessageEnvironmentStateData(item.getObject()));
                }
            }
        } catch (final RuntimeException ex) {
            log.error("Error al consultar estados de entorno de mensaje en SurrealDB: " + sql, ex);
            throw ex;
        }
        return list;
    }

    private static MessageEnvironmentStateData toMessageEnvironmentStateData(final Object obj) {
