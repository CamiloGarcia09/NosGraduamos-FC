...
@Repository
public class MessageTypeCatalogSurrealAdapter implements MessageTypeCatalogRepository {
    ...
    public MessageTypeCatalogSurrealAdapter(final Surreal surreal, final LoggingPortFactory loggerFactory) {
        this.log = loggerFactory.getLogger(MessageTypeCatalogSurrealAdapter.class);
        this.surreal = surreal;
    }

    @Override
    public List<MessageTypeData> findAll() {
        final String sql = "SELECT * FROM " + SURREAL_TABLE_MESSAGE_TYPE + ";";
        final List<MessageTypeData> list = new ArrayList<>();
        try {
            ...
            for (int i = 0; i < array.len(); i++) {
                ...
                if (!isNullObject(item) && item.isObject()) {
                    list.add(toMessageTypeData(item.getObject()));
                }
            }
        } catch (final RuntimeException ex) {
            log.error("Error al consultar tipos de mensaje en SurrealDB: " + sql, ex);
            throw ex;
        }
        return list;
    }

    private static MessageTypeData toMessageTypeData(final Object obj) {
