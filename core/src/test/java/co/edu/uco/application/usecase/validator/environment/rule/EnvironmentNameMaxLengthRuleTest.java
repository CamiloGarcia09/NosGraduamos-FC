...
class EnvironmentNameMaxLengthRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private EnvironmentNameMaxLengthRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new EnvironmentNameMaxLengthRule(catalogPort);
    }

    @Test
    void validate_doesNotThrow_whenNameDoesNotExceedMaximum() {
        CreateEnvironmentDTO dto = CreateEnvironmentDTO.builder().name("Production").build();
    ...
    void validate_throwsBusinessRuleException_whenNameExceedsMaximum() {
        CreateEnvironmentDTO dto = CreateEnvironmentDTO.builder().name("a".repeat(51)).build();
