...
class MessageFunctionalityIdRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private MessageFunctionalityIdRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new MessageFunctionalityIdRequiredRule(catalogPort);
    }

    @Test
    void validate_doesNotThrow_whenFunctionalityIdIsPresent() {
        CreateMessageDTO dto = CreateMessageDTO.builder().functionalityId("func-1").build();

        assertDoesNotThrow(() -> rule.validate(dto));
    }

    @Test
    void validate_throwsBusinessRuleException_whenFunctionalityIdIsEmpty() {
