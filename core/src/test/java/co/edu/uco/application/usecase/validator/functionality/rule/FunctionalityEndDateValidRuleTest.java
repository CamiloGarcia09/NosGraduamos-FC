...
class FunctionalityEndDateValidRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private FunctionalityEndDateValidRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new FunctionalityEndDateValidRule(catalogPort);
    }
    ...
    void validate_throwsBusinessRuleException_whenEndDateIsInvalid() {
        CreateFunctionalityDTO dto = CreateFunctionalityDTO.builder().endDate("not-a-date").build();
