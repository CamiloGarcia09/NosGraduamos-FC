...
class FunctionalityStartDateRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private FunctionalityStartDateRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new FunctionalityStartDateRequiredRule(catalogPort);
    }

    @Test
    void validate_doesNotThrow_whenStartDateIsPresent() {
        CreateFunctionalityDTO dto = CreateFunctionalityDTO.builder().startDate("2025-01-01T00:00:00").build();

        assertDoesNotThrow(() -> rule.validate(dto));
    }

    @Test
    void validate_throwsBusinessRuleException_whenStartDateIsEmpty() {
