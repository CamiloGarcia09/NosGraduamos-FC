...
class TokenEnvironmentUuidRuleTest {
    ...
    private TokenEnvironmentUuidRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new TokenEnvironmentUuidRule(catalogPort);
    }

    private TokenValidationContext context(String environmentId) {
        CreateTokenDTO dto = new CreateTokenDTO("2026-01-01T00:00:00", environmentId);
        return new TokenValidationContext(dto, APPLICATION_ID);
    }

    @Test
    void validate_doesNotThrow_whenEnvironmentIdIsValidUuid() {
        assertDoesNotThrow(() -> rule.validate(context(ENVIRONMENT_ID)));
    }

    @Test
    void validate_throwsBusinessRuleException_whenEnvironmentIdIsNotAUuid() {
