...
class EnvironmentStateExistsRuleTest {
    ...
    private EnvironmentStateExistsRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new EnvironmentStateExistsRule(catalogPort, recordExistsCatalogPort);
    }
    ...
    void validate_throwsBusinessRuleException_whenStateDoesNotExist() {
        ...
        CreateEnvironmentDTO dto = CreateEnvironmentDTO.builder().stateId("unknown").build();
