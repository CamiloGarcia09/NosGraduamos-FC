...
class FunctionalityEndDateRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private FunctionalityEndDateRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new FunctionalityEndDateRequiredRule(catalogPort);
    }

    @Test
    void validate_doesNotThrow_whenEndDateIsPresent() {
        CreateFunctionalityDTO dto = CreateFunctionalityDTO.builder().endDate("2025-12-31T23:59:59").build();

        assertDoesNotThrow(() -> rule.validate(dto));
    }

    @Test
    void validate_throwsBusinessRuleException_whenEndDateIsEmpty() {
