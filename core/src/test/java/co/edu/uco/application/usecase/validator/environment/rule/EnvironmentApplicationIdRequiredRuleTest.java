...
class EnvironmentApplicationIdRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private EnvironmentApplicationIdRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new EnvironmentApplicationIdRequiredRule(catalogPort);
    }
