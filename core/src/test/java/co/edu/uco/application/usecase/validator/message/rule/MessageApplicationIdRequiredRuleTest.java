...
class MessageApplicationIdRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private MessageApplicationIdRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new MessageApplicationIdRequiredRule(catalogPort);
    }

    @Test
    void validate_doesNotThrow_whenApplicationIdIsPresent() {
        CreateMessageDTO dto = CreateMessageDTO.builder().applicationId("app-1").build();
