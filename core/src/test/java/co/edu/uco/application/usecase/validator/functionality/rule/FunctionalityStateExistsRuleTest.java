...
class FunctionalityStateExistsRuleTest {
    ...
    private FunctionalityStateExistsRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new FunctionalityStateExistsRule(catalogPort, recordExistsCatalogPort);
    }

    @Test
    void validate_doesNotThrow_whenStateExists() {
        when(recordExistsCatalogPort.exists(any(), anyString())).thenReturn(true);
        CreateFunctionalityDTO dto = CreateFunctionalityDTO.builder().stateId("state-1").build();

        assertDoesNotThrow(() -> rule.validate(dto));
    }

    @Test
    void validate_throwsBusinessRuleException_whenStateDoesNotExist() {
        when(recordExistsCatalogPort.exists(any(), anyString())).thenReturn(false);
        CreateFunctionalityDTO dto = CreateFunctionalityDTO.builder().stateId("unknown").build();
