...
class FunctionalityStartDateValidRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private FunctionalityStartDateValidRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new FunctionalityStartDateValidRule(catalogPort);
    }
    ...
    void validate_throwsBusinessRuleException_whenStartDateIsInvalid() {
        CreateFunctionalityDTO dto = CreateFunctionalityDTO.builder().startDate("not-a-date").build();
