...
class MessageTitleRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private MessageTitleRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new MessageTitleRequiredRule(catalogPort);
    }

    @Test
    void validate_doesNotThrow_whenTitleIsPresent() {
    ...
    @Test
    void validate_throwsBusinessRuleException_whenTitleIsEmpty() {
