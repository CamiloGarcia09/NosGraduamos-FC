...
class EnvironmentNameRequiredRuleTest {

    @Mock
    private CatalogPort catalogPort;

    private EnvironmentNameRequiredRule rule;

    @BeforeEach
    void setUp() {
        lenient().when(catalogPort.getMessage(anyString())).thenReturn("user message");
        rule = new EnvironmentNameRequiredRule(catalogPort);
    }
    ...
    @Test
    void validate_throwsBusinessRuleException_whenNameIsEmpty() {
